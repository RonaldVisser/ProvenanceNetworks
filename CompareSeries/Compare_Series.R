...
source("wuchswerte.R")
source("read_fh_BC.R")
source("cor_mat_overlap.R")
source("t_value.R")

# select tree ring with series to compare
treeringfile <- file.choose()
treeringfile2 <- file.choose()
# open files
trs1 <- read_fh_BC(treeringfile)
trs2 <- read_fh_BC(treeringfile2)
# nr of series in files
n1 <- dim(trs1)[2]
n2 <- dim(trs2)[2]
...
# minimal overlap for comparison
overlap <- 50
...
r_list <- cor_mat_overlap(trs1,trs2,overlap)
r_mat <- r_list[[1]]
r_ol_mat <- r_list[[2]]
rm(r_list)
rhol_list <- cor_mat_overlap(trs1_wuchs,trs2_wuchs,overlap)
...
# remove lists
rm(list_overlap,list_SGC,list_SSGC, list_p,list_r,list_r_ol,list_rhol,list_t,list_thol)
